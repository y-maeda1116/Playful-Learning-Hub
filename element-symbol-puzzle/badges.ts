 ...
 */
export function getBadgesByGrade(gradeLevel: 3 | 4 | 5 | 6): Badge[] {
  return Object.values(BADGES).filter((badge) => badge.gradeLevel <= gradeLevel);
}
...
export function getBadgeById(badgeId: string): Badge | undefined {
  return BADGES[badgeId.toUpperCase().replace(/-/g, '_')] || Object.values(BADGES).find((b) => b.id === badgeId);
}
...
export function getBadgesByType(
  ...
  return Object.values(BADGES).filter((badge) => badge.criteria.type === type);
}
 ...
 * @returns The badge object or undefined if not found
 */
export function getBadgeByTypeAndThreshold(
  type: 'elements-learned' | 'accuracy' | 'streak' | 'formula-mastery' | 'jigsaw-completed',
  threshold: number
): Badge | undefined {
  return Object.values(BADGES).find((badge) => badge.criteria.type === type && badge.criteria.threshold === threshold);
}
