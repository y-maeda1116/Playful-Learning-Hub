...
import { Badge, UserProgress, GameSession, Achievement } from '../data/types';
import { BADGES, getBadgeById, getBadgesByType } from '../data/badges';
...
export class RewardSystem {
   ...
   */
  private checkBadgeCriteriaForBadge(userId: string, badge: Badge): boolean {
    const progress = this.progressTrackingSystem.getProgress(userId);

    switch (badge.criteria.type) {
      case 'elements-learned': {
        const masteredCount = Object.values(progress.elementsLearned).filter(
          (elem) => elem.status === 'mastered',
        ).length;
