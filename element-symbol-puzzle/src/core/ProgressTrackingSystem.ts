...
import { UserProgress, GameSession, Answer } from '../data/types';
...
export class ProgressTrackingSystem {
   ...
   */
  private initializeUserProgress(userId: string, gradeLevel: 3 | 4 | 5 | 6): UserProgress {
  ...
  updateProgress(userId: string, sessionData: GameSession): void {
    ...
    const totalAnswers = Object.values(progress.elementsLearned).reduce(
      (sum, elem) => sum + elem.attempts,
      0,
    );
    const totalCorrect = Object.values(progress.elementsLearned).reduce(
      (sum, elem) => sum + elem.correctAttempts,
      0,
    );
  ...
  loadFromLocalStorage(userId: string, gradeLevel: 3 | 4 | 5 | 6): UserProgress {
    ...
    try {
      ...
      const progress = JSON.parse(storedData) as UserProgress;
