...
const ElementContentManager_1 = require("./ElementContentManager");
const DifficultyAdjustmentSystem_1 = require("./DifficultyAdjustmentSystem");
const AudioSystem_1 = require("./AudioSystem");
/**
 * GameEngine class manages game sessions and game logic
 */
class GameEngine {
    constructor(difficultySystem, audioSystem) {
        this.sessions = new Map();
        this.answerStartTimes = new Map(); // Track when answer submission started
        this.sessionUserIds = new Map(); // Track userId for each session
        this.contentManager = new ElementContentManager_1.ElementContentManager();
        this.difficultySystem = difficultySystem || new DifficultyAdjustmentSystem_1.DifficultyAdjustmentSystem();
     ...
     */
    startSession(gameType, gradeLevel, userId) {
     ...
     */
    startMatchingSession(gradeLevel, userId) {
     ...
     */
    startQuizSession(gradeLevel, userId) {
     ...
     */
    adjustDifficultyAfterSession(sessionId) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
     ...
     */
    submitQuizAnswer(sessionId, userAnswer) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
     ...
     */
    async playElementAudio(element) {
        try {
     ...
     * @returns A promise that resolves when audio playback completes or fails gracefully
     */
    async playCorrectFeedbackAudio() {
        try {
     ...
     * @returns A promise that resolves when audio playback completes or fails gracefully
     */
    async playIncorrectFeedbackAudio() {
        try {
     ...
     * @returns A promise that resolves when audio playback completes or fails gracefully
     */
    async playRewardNotificationAudio() {
        try {
