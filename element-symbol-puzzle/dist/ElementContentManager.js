...
exports.ElementContentManager = void 0;
...
class ElementContentManager {
     ...
     */
    constructor() {
        this.elements = elements_1.ELEMENTS;
        this.chemicalFormulas = elements_1.CHEMICAL_FORMULAS;
    }
    /**
     ...
     * // Returns true
     */
    validateFormulaComposition(formulaId, selectedElementIds) {
     ...
     */
    getRandomElementsByGrade(grade, count) {
        const gradeElements = this.getElementsByGrade(grade);
        if (count > gradeElements.length) {
            throw new Error(`Cannot get ${count} random elements. Only ${gradeElements.length} elements available for grade ${grade}.`);
        }
     ...
     */
    getRandomFormulasByGrade(grade, count) {
        const gradeFormulas = this.getFormulasByGrade(grade);
        if (count > gradeFormulas.length) {
            throw new Error(`Cannot get ${count} random formulas. Only ${gradeFormulas.length} formulas available for grade ${grade}.`);
        }
        // Fisher-Yates shuffle
...
exports.ElementContentManager = ElementContentManager;
